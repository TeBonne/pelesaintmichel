...
export async function getSimplePage(slug) {
  const s = await createClient();
  const [page, chrome] = await Promise.all([
    (async () => (await s.from("simple_pages").select("*").eq("slug", slug).maybeSingle()).data || null)(),
    getChrome(),
  ]);
  return { page, ...chrome };
}
...
export async function getHomeContent() {
  const supabase = await createClient();
  const [hero, stats, programme, citation, videos, faq, settings, nav, testimonials] = await Promise.all([
