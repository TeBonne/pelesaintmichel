    ...
    <section id="inscription" style={{ background: "var(--ivory)", padding: "72px 32px 90px" }}>
      <div style={{ maxWidth: 640, margin: "0 auto" }}>
        <div style={{ textAlign: "center", marginBottom: 18 }}>
          ...
          <h2 style={{ fontFamily: "var(--font-serif)", fontWeight: 600, fontSize: 38, color: "var(--navy-700)", margin: "10px 0 0" }}>
            Rejoignez la marche
          </h2>
        </div>

        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8, margin: "22px 0 32px" }}>
          {[1, 2, 3].map((n) => (
            <Fragment key={n}>
              <div
                style={{
                  ...
                  border: step >= n ? "none" : "1px solid var(--stone-300)",
                }}
              >
                {n}
              </div>
              {n < 3 && <div style={{ width: 48, height: 2, background: step > n ? "var(--gold-500)" : "var(--stone-200)" }} />}
            </Fragment>
          ))}
        </div>

        <div style={{ background: "var(--parchment)", border: "1px solid var(--line)", borderTop: "3px solid var(--gold-500)", borderRadius: "var(--r-md)", boxShadow: "var(--shadow-md)", padding: "34px 36px" }}>
          {step === 1 && (
          ...
          {step === 3 && (
            <div style={{ textAlign: "center", padding: "12px 0" }}>
              ...
              <h3 style={{ fontFamily: "var(--font-serif)", fontWeight: 600, fontSize: 26, color: "var(--navy-700)", margin: "20px 0 8px" }}>Récapitulatif</h3>
              <p style={{ fontFamily: "var(--font-body)", fontSize: 17, lineHeight: 1.7, color: "var(--ink-soft)", margin: "0 auto", maxWidth: 420 }}>
                {form.prenom || "Pèlerin"} {form.nom}, branche <b style={{ color: "var(--navy-700)" }}>{form.branche.toLowerCase()}</b>,{" "}
                {form.venue.toLowerCase()}
                {form.chapitre ? ` (chapitre ${form.chapitre})` : ""}. Au départ, un livret du pèlerin vous sera remis ; à l'arrivée, le plomb de Saint-Michel.
              </p>
            </div>
          )}

          <div style={{ display: "flex", justifyContent: "space-between", marginTop: 30, gap: 12 }}>
            <Button variant="ghost" onClick={() => setStep(Math.max(1, step - 1))} style={{ opacity: step === 1 ? 0.4 : 1, pointerEvents: step === 1 ? "none" : "auto" }}>
              Retour
            </Button>
            {step < total ? (
